...
const OPENROUTER_API_KEY = "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"; // Replace with your sk-or-v1-... key
...
function processFile(file) {
    ...
    reader.onload = (e) => {
        currentDataUrl = e.target.result; // Complete base64 Data URL
...
/**
 * Executes Fabric Analysis using OpenRouter Free Models
 */
async function performFabricScan() {
    if (!currentDataUrl) return;

    if (!OPENROUTER_API_KEY || OPENROUTER_API_KEY === "YOUR_OPENROUTER_API_KEY_HERE") {
        showError("API Key missing. Please paste your OpenRouter key into script.js.");
        return;
    }
    ...
    const promptText = `
    Analyze this fabric image and respond strictly with a valid JSON object matching this exact structure:
    {
    ...
    Do not wrap the response in markdown code blocks like \`\`\`json. Return pure raw JSON text only.
    `;

    const requestBody = {
        model: "openrouter/free", // Automatically routes to available free vision models
        messages: [
            {
                role: "user",
                content: [
